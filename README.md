...
> **Status: deployed and verified live** on GKE Autopilot (europe-west1), 2026-09-25/26. `./scripts/up.sh` built it end to end and the live suite [`scripts/test.sh`](scripts/test.sh) passes **28 of 28** (results: [`docs/test-results.md`](docs/test-results.md)). The bad-release canary drill and rollback ran for real ([runbook 05](docs/runbooks/05-release-and-rollback.md)). Getting there took **seven fixes** that only a live deployment could find — each is documented in the [troubleshooting runbook](docs/runbooks/09-troubleshooting.md) (Autopilot limits, GKE-injected fields, NodeLocal DNS vs default-deny, a Prometheus namespace, build-tool tracks, …). Evidence below was captured from that live deployment, which was then removed with `./scripts/down.sh` (94 resources destroyed, nothing billable left — [runbook 01](docs/runbooks/01-build-and-teardown.md)).
